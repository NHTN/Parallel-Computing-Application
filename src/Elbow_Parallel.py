...
def Calculate_SSE(out_img, img):
    ...
    return np.reshape(SSE_list, (len(SSE_list) *  len(SSE_list[0])*  len(SSE_list[0][0]), 1)).sum()
    ...
    for i in range(n):
        ...
        if(i > 2):
            for j in range(len(SSE) - 3):
                temp = int(SSE[j+1] - SSE[j+2])
                if (temp == 0):
                    temp = 1
                if(abs(int(SSE[j] - SSE[j+1]) / temp) < 1.4):
                    return j + 1
...
def main():
    ...
    Calc_K(Resize_Image(args.filename1))
